...
class InsuranceClaimTest {
    ...
    @CsvSource({"Approved, 850.0", "Rejected, 0", "Pending,  0"})
    @DisplayName("Parameterized: calculatePayout cho các trạng thái khác nhau")
    void calculatePayout_VariousStatuses(String status, double expectedPayout) {         // Arrange
    ...
    void toString_ReturnsExpectedFormat() {
        ...
        // Assert
        assertAll(() -> assertTrue(output.contains("InsuranceClaim")), () -> assertTrue(output.contains("claimId='C001'")), () -> assertTrue(output.contains("amount=1000.0")), () -> assertTrue(output.contains("claimStatus='Pending'")));
    }
}
