...
class AccountServiceTest {
    ...
    @ValueSource(strings = {"john@example.com", "alice.b@mail.co.uk", "carol_99@domain.io"})
    ...
    @CsvSource(value = {"bobmail.com",        // thiếu @
            "missing@dot",        // thiếu .domain
            "'@nodomain.com'",    // thiếu local part
            "' '",                // chỉ khoảng trắng             "NULL"                // sẽ map về null
    }, nullValues = "NULL")
    ...
    void registerAccount_FromCsv(String username, String password, String email, boolean expected) {
        ...
        // Assert
        assertEquals(expected, actual, () -> String.format("(%s,%s,%s) phải trả về %s", username, password, email, expected));
    }
